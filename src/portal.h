...
public:
    ...
    void start(const char* mdns = "", uint16_t port = 80) {
        ...
        #ifndef GP_NO_DNS
        if (_dns) dnsServer.start(53, "rc.local", WiFi.softAPIP());
        #endif

        server.onNotFound([this]()
        {
            if (_preNotFound && _preNotFound()) return;                         //если _preNotFound() вернул true - запрос обработан -> на выход
            ...
            _showPage = 0;
            _uri = server.uri();
            if (_uri.startsWith(F("/GP_update"))) {                             // апдейт
    ...
    // хук для onNotFound - будет вызываться первым перед всеми неназначенными роутами
    void attachPreNotFound(bool (*handler)()) { _preNotFound = handler; }
